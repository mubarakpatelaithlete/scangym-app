...
function initVisitorCounter(){
  ...
  /* Round 5: /api/stats/live-visitors now returns REAL 5-minute visitor counts.
   * Honest social proof (Booking.com science): only swap the ticker text when
   * the server sends a label (count >= 5) — a quiet moment keeps the static
   * "No membership needed" USP instead of "1 person here". */
  async function f(){try{var r=await fetch('/api/stats/live-visitors');if(!r.ok||((r.headers.get('content-type')||'').indexOf('json')===-1))return;var d=await r.json();var el=document.getElementById('sg-lvt');if(el&&d.label)el.textContent=d.label.replace(/^\u{1F525}\s*/u,'');}catch(e){}}
