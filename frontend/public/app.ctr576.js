...
const FUN_FACTS=[
  'ScanGym lets you book any gym near you — pay per session, no membership needed',
...
async function loadConfig() {
  try {
    ...
    // Show real gym count from DB (honest number)
    ...
    ${(!isLoading&&gyms[0])?(function(){
        ...
        _cards.forEach(function(c,i){
          ...
          if(i===0){
            ...
            html+='<button onclick="sgToggleFilter(this,\'near\')" class="sg-filter-pill" data-filter="near">Nearest First</button>';
...
window.openGymOverlay=function(section){
  ...
  if(section==='reviews'){
    ...
    // H10 fix: show honest message instead of fabricated equipment lists
    const gymName = gym.name || 'This gym';
    const phoneLink = gym.formatted_phone_number
      ? `<a href="tel:${gym.formatted_phone_number}" style="color:#FF6D00;text-decoration:none">${gym.formatted_phone_number}</a>`
      : '';
    const websiteLink = gym.website
      ? `<a href="${gym.website}" target="_blank" rel="noopener" style="color:#FF6D00;text-decoration:none">Visit website →</a>`
      : '';
    body.innerHTML=`
      <div style="text-align:center;padding:24px 16px">
        <div style="font-size:48px;margin-bottom:12px">🏋️</div>
        <p style="color:#fff;font-size:15px;font-weight:600;margin-bottom:8px">Equipment details</p>
        <p style="color:rgba(255,255,255,.5);font-size:13px;line-height:1.6;margin-bottom:16px">
          Contact <strong style="color:rgba(255,255,255,.7)">${gymName}</strong> directly for their full equipment list and facilities.
        </p>
        <div style="display:flex;flex-direction:column;gap:8px;align-items:center">
          ${phoneLink ? '<p style="font-size:14px">📞 '+phoneLink+'</p>' : ''}
          ${websiteLink ? '<p style="font-size:14px">🌐 '+websiteLink+'</p>' : ''}
        </div>
      </div>
    `;
  }
  ...
  <div class="pt-8 min-h-full px-4 flex items-center justify-center">
    <div class="max-w-md w-full">
      <div class="text-center mb-8">
        ...
        <p class="text-slate-400 text-sm mt-1">${isCodeStep ? 'Enter the code we sent to '+state.authPhone : 'One account. Any gym near you.'}</p>
      </div>
  ...
  <div class="pt-16 min-h-full px-4 pb-8">
    <div class="max-w-lg mx-auto">
      ...
      <div class="text-center mb-6 fade-in">
        ...
        <p class="text-slate-500 text-xs mt-2">🌍 Your ScanGym profile is accepted at any gym near you</p>
      </div>
...
// ═══════════════════════════════════════════════════════════════════
//  UNIVERSAL SCANGYM PROFILE — "One signup. Any gym near you."
// ═══════════════════════════════════════════════════════════════════
  ...
  return`<div style="background:linear-gradient(135deg,rgba(255,109,0,.12),rgba(255,109,0,.04));border:1px solid rgba(255,109,0,.2);border-radius:20px;padding:20px;margin-bottom:20px;position:relative;overflow:hidden">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between">
      <div style="display:flex;align-items:center;gap:6px">
        ...
        <span style="color:rgba(255,255,255,.5);font-size:11px;font-weight:600">Accepted at any gym near you</span>
      </div>
...
window._shareIDCard=function(){
  if(navigator.share){
    navigator.share({title:'My ScanGym ID',text:'Check out my ScanGym gym pass! Book any gym, pay per session — no membership needed.',url:'https://scangym.com'}).catch(()=>{});
  }else{
...
function ProfilePage(){
  ...
  return`<div style="padding:16px;max-width:480px;margin:0 auto">
    ...
    <div style="margin-top:24px;text-align:center">
      <p style="color:rgba(255,255,255,.3);font-size:12px;line-height:1.6">🌍 Your ScanGym profile is your universal gym pass.<br>Book any gym near you — pay per session, no membership.</p>
    </div>
  </div>`;
}
