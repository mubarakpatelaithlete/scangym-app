        ...
        if(_cards.length>_INITIAL_CARDS){
          setTimeout(function(){
            ...
            _cards.slice(_INITIAL_CARDS).forEach(function(c,idx){
              ...
              cardHtml+='<div class="tt-action" onclick="event.stopPropagation();openGymDirectOverlay(\''+c.id+'\',true,\'passes\')"><div class="tt-action-btn">\u{1F39F}\uFE0F</div></div>';
              cardHtml+='<div class="tt-action" onclick="event.stopPropagation();showUberCheckout(\''+c.id+'\')"><div class="tt-action-btn">\u{1F4C5}</div></div>';
              cardHtml+='<div class="tt-action" onclick="event.stopPropagation();openGymDirectOverlay(\''+c.id+'\',true,\'facilities\')"><div class="tt-action-btn">\u{1F3CA}</div></div>';
              cardHtml+='<div class="tt-action" onclick="event.stopPropagation();openGymDirectOverlay(\''+c.id+'\',true,\'equipment\')"><div class="tt-action-btn">\u{1F3CB}\uFE0F</div></div>';
              cardHtml+='</div>';
              ...
              var _bT=bookedToday(c.name);var _pL=peopleLooking(c.name);
              ...
              cardHtml+='<div class="tt-chip" style="background:rgba(34,197,94,.18);border:1px solid rgba(34,197,94,.25)">\u{1F4C8} '+_bT+' booked today</div>';
              cardHtml+='<div class="tt-chip" style="background:rgba(239,68,68,.14);border:1px solid rgba(239,68,68,.2)">\u{1F440} '+_pL+' looking now</div>';
              cardHtml+='</div>';
