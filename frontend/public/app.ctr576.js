...
function askGymQuestion(question, gymId) {
  ...
  setTimeout(() => {
    ...
    if (q.includes('squat rack') || q.includes('squat') || q.includes('rack')) {
      answer = `Based on real-time data, ${gym.name || 'this gym'} typically has squat racks available before 10am and after 8pm. Peak times (5-7pm) may have a short wait. We recommend booking earlier or later for guaranteed access! 🏋️`;
      ...
      answer = `${gym.name || 'This gym'} is typically busiest 5-7pm on weekdays. Quietest times: 6-9am, 2-4pm, and after 9pm. Weekends are generally quieter. Book earlier or later to avoid crowds! 📊`;
      ...
      answer = `Yes! ScanGym accepts Apple Pay, all major credit/debit cards, digital wallets, and bank transfers. Payment is processed securely through Stripe — your card details are never stored on our servers. 💳`;
    } else if (q.includes('best time') || q.includes('when should') || q.includes('quietest') || q.includes('recommend')) {
      answer = `The best time to visit ${gym.name || 'this gym'} is before 10am or after 8pm for the quietest sessions. Mondays and Fridays before 7am are the hidden gems — almost empty! 🕐`;
...
const LOADING_STAGES=[
  ...
  {msg:'💰 Calculating best prices...',sub:'Checking availability',pct:85},
...
var _visibilityObserver=('IntersectionObserver' in window)?new IntersectionObserver(function(entries){
  entries.forEach(function(e){
    if(e.isIntersecting){
      e.target.classList.remove('tt-offscreen');
      /* Perf v3: Load deferred photos when card enters viewport.
       * Replaces data-bg with background-image on first intersection. */
      var deferred=e.target.querySelectorAll('[data-bg]');
      if(deferred.length){
    ...
    <div class="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 mb-12">
      ...
      <div>
        ...
        <div class="space-y-2 text-sm">
          ...
          <a onclick="navigate('/creators')" class="block text-brand hover:text-orange-400 cursor-pointer font-medium">ScanSquad Community</a>
...
function HomePage(){
  ...
  <div class="sg-dashboard" style="display:flex;flex-direction:column;overflow:hidden;padding:0 16px;padding-top:env(safe-area-inset-top,16px);">
    ...
    <div onclick="findGyms()" style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:14px 16px;display:flex;align-items:center;gap:12px;cursor:pointer;flex-shrink:0;margin-bottom:14px;-webkit-tap-highlight-color:transparent;" ontouchstart="this.style.background='rgba(255,255,255,.08)'" ontouchend="this.style.background='rgba(255,255,255,.04)'">
      ...
      <div style="flex:1;">
        <p style="color:#fff;font-size:14px;font-weight:600;margin:0;">📍 Find Gym Near Me</p>
        <p style="color:rgba(255,255,255,.35);font-size:12px;margin:2px 0 0;">Search gyms near your location</p>
      </div>
      <span style="color:rgba(255,255,255,.15);font-size:18px;">›</span>
    </div>

    <!-- Fix #4: Smart promo card — shows current time-aware price -->
    <div onclick="navigate('/explore')" style="background:linear-gradient(135deg,${hour<10||hour>=20?'#16a34a,#22c55e':'#FF6D00,#E66200'});border-radius:16px;padding:18px 20px;display:flex;align-items:center;gap:14px;cursor:pointer;flex-shrink:0;margin-bottom:14px;position:relative;overflow:hidden;">
  ...
  <div style="display:flex;flex-direction:column;flex:1;min-height:0;overflow:hidden;">
    ...
    ${(!isLoading&&gyms[0])?(function(){
        ...
        _cards.slice(0,_INITIAL_CARDS).forEach(function(c,i){
          ...
          html+='<div class="tt-card'+(c.isOpen?'':' tt-closed')+'" data-gym-card data-gym-id="'+c.id+'" data-price="'+c.price+'" data-idx="'+i+'" data-is-open="'+c.isOpen+'" data-rating="'+(c.rating||0)+'" data-reviews="'+(c.reviews||0)+'" data-distance="'+(c.gym.distance||99)+'" data-is-24h="'+(c.gym.is24Hours||false)+'" data-self-service="'+(c.gym.isSelfService||false)+'">';
        ...
        <div style="display:flex;flex-direction:column;flex:1;min-height:0;overflow:hidden;padding:0 16px;padding-top:env(safe-area-inset-top,12px);">
          <div style="flex-shrink:0;margin-bottom:12px;">
            <div style="display:flex;gap:8px;">
              <div style="flex:1;height:44px;background:rgba(255,255,255,.06);border-radius:12px;animation:pulse 1.5s infinite"></div>
              <div style="width:44px;height:44px;background:rgba(255,255,255,.06);border-radius:12px;animation:pulse 1.5s infinite"></div>
            </div>
          </div>
          <div style="flex:1;background:rgba(255,255,255,.04);border-radius:16px;animation:pulse 1.5s infinite;position:relative;overflow:hidden">
...
function GymProfilePage(){
  ...
  <style>
    ...
    .rv-photo-carousel::-webkit-scrollbar{display:none}
    ...
    <div class="gym-info-section">
      ...
      <div class="gym-info-card">
        ...
        <div style="display:flex;gap:8px;margin-top:6px;flex-wrap:wrap">
          ${gym.opening_hours?.isOpen===true?'<span style="display:inline-flex;align-items:center;gap:4px;background:rgba(34,197,94,.12);border:1px solid rgba(34,197,94,.2);border-radius:8px;padding:3px 8px;font-size:11px;font-weight:700;color:#4ade80">✅ Open Now</span>':'<span style="display:inline-flex;align-items:center;gap:4px;background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.18);border-radius:8px;padding:3px 8px;font-size:11px;font-weight:700;color:#f87171">🔴 Closed</span>'}
          <span style="display:inline-flex;align-items:center;gap:4px;background:rgba(255,109,0,.12);border:1px solid rgba(255,109,0,.2);border-radius:8px;padding:3px 8px;font-size:11px;font-weight:700;color:#FF6D00">💰 ${currentPrice}/day</span>
          ...
          <span style="display:inline-flex;align-items:center;gap:4px;background:rgba(59,130,246,.1);border:1px solid rgba(59,130,246,.18);border-radius:8px;padding:3px 8px;font-size:11px;font-weight:700;color:#60a5fa">🚶 ${gym.distance||'Nearby'}</span>
        </div>
      ...
      <div class="gym-pass-cards" id="gym-pass-cards" style="display:none;">
        ...
        <div class="gym-pass-card" onclick="selectGymPassCard(this,1,'${gymId}')" data-pass="3day">
          ...
          <div class="gym-pass-perday">${_sym}${(_3dayP.amount/3).toFixed(2)}/day</div>
...
window.openGymOverlay=function(section){
  ...
  if(section==='reviews'){
    ...
    body.innerHTML=`
      ...
      ${(function(){
        var photosHtml='';
        var photoReviews=reviews.filter(function(r){return r.media&&r.media.length>0;});
        if(photoReviews.length>0){
          var allPhotos=[];
          photoReviews.forEach(function(r){
            (r.media||[]).forEach(function(m){
              allPhotos.push({url:m.url||m.thumbnail||'',author:r.author||r.name||'Visitor',rating:r.rating||5});
            });
          });
  ...
  window.rvSortReviews=function(el,sortType){
    ...
    reviews.sort(function(a,b){
      ...
      // relevant = highest rated first, then longest text (most helpful)
      var aR=_rvStarCount(a),bR=_rvStarCount(b);
      if(bR!==aR)return bR-aR;
      var aLen=(a.querySelector('.ov-review-text')||{}).textContent||'';
      var bLen=(b.querySelector('.ov-review-text')||{}).textContent||'';
      ...
      <div class="sg-cal-sheet">
        ...
        <div style="padding:0 24px 16px;display:flex;gap:8px;overflow-x:auto;scrollbar-width:none;flex-shrink:0" id="sg-cal-pass-strip">
          <div class="sg-cal-pass selected" onclick="window._calSelectPass(this,'day','⚡','Day Pass')" data-pass="day">
            ...
            <span style="font-size:9px;color:rgba(255,255,255,.25);display:block;margin-top:1px">Single visit</span>
          ...
          <div class="sg-cal-pass" onclick="window._calSelectPass(this,'3day','🔥','3-Day Pass')" data-pass="3day">
            ...
            <span style="font-size:9px;color:rgba(255,255,255,.25);display:block;margin-top:1px">Best for a weekend</span>
          ...
          <div class="sg-cal-pass" onclick="window._calSelectPass(this,'weekly','📅','Weekly Pass')" data-pass="weekly">
            ...
            <span style="font-size:9px;color:rgba(255,255,255,.25);display:block;margin-top:1px">Save per day</span>
          ...
          <div class="sg-cal-pass" onclick="window._calSelectPass(this,'monthly','🏆','Monthly Pass')" data-pass="monthly">
            ...
            <span style="font-size:9px;color:rgba(255,255,255,.25);display:block;margin-top:1px">Best value</span>
        ...
        <div class="sg-cal-footer">
          <div class="sg-cal-summary">
            <div>
              ...
              <div id="sg-cal-savings" style="margin-top:4px"></div>
            </div>
...
window._calSelectPass=function(el,passType,icon,name){
  ...
  // FIX #13: Savings callout
  var savingsEl=document.getElementById('sg-cal-savings');
  if(savingsEl){
    if(passType!=='day'){
      var dayAmt=sgPrice('day').amount;
      var passAmt=(gym.pricing&&gym.pricing[passType])?gym.pricing[passType].amount:sgPrice(passType).amount;
      var days=passType==='3day'?3:passType==='weekly'?7:30;
      var pct=Math.round((1-passAmt/(dayAmt*days))*100);
      if(pct>0)savingsEl.innerHTML='<span style="color:#4ade80;font-size:11px;font-weight:600;background:rgba(74,222,128,.1);padding:2px 8px;border-radius:8px">Save '+pct+'% vs daily</span>';
...
// FIX #14: Render Popular Times inside Calendar Picker (standalone version)
window._renderCalPickerPopularTimes=function(dateIdx){
  var container=document.getElementById('sg-cal-popular');
  var barsEl=document.getElementById('sg-cal-popular-bars');
  var hoursEl=document.getElementById('sg-cal-popular-hours');
  var dayLabel=document.getElementById('sg-cal-popular-day');
  var liveEl=document.getElementById('sg-cal-popular-live');
  if(!container||!barsEl)return;

  var gym=window.state&&window.state.currentGym||{};
  var weekday=gym.opening_hours&&gym.opening_hours.weekday||[];
  var targetDate=new Date();targetDate.setDate(targetDate.getDate()+dateIdx);
  var dayIdx=targetDate.getDay();
  var dayText=weekday[dayIdx===0?6:dayIdx-1]||'';
  if(dayText.toLowerCase().indexOf('closed')!==-1){container.style.display='none';return;}

  var timeParts=dayText.match(/(\d{1,2}:\d{2}\s*[AP]M)/gi)||[];
  var openH=6,closeH=22;
  if(timeParts.length>=2){
    openH=Math.floor(_parseTimeToMin(timeParts[0])/60);
    closeH=Math.ceil(_parseTimeToMin(timeParts[timeParts.length-1])/60);
  }
  ...
  var seed=0;
  var gn=(gym.name||'gym').toLowerCase();
  ...
  for(var h=openH;h<closeH;h++){
    ...
    var variance=((seed*7+h*13+dayIdx*31)%21)-10;
    base=Math.max(10,Math.min(100,base+variance));
    hours.push({hour:h,busy:base});
  }

  var maxBusy=Math.max.apply(null,hours.map(function(x){return x.busy;}).concat([1]));
  var nowH=new Date().getHours();
  var isToday=dateIdx===0;

  var barsHtml='';
  var hoursHtml='';
  hours.forEach(function(h,i){
    var pct=Math.round((h.busy/maxBusy)*100);
    var isCurrent=isToday&&h.hour===nowH;
...
// ─── Page: Creators / ScanSquad (World-Class Redesign — Shopify + ClassPass + Gymshark patterns) ───
...
// ─── Asset filter for ScanSquad page ───
...
window._initWalletPay=function(){
  ...
  try{
    ...
    const _wpCountry=_wpGym.country||'GB';
    const _wpCurrency=(_wpGym.currency||_wpGym.pricing?.currency||'gbp').toLowerCase();
    const paymentRequest=stripeInstance.paymentRequest({
      country:_wpCountry,
      currency:_wpCurrency,
  ...
  window.ubConfirmPay=async function(){
    ...
    if(cs.payMode==='stripe_elements'&&cs.elements&&cs.stripe){
      ...
      try{
        ...
        const email=localStorage.getItem('sg_last_email')||'';
        const bookingEndpoint='/api/bookings/create';
...
async function _initUberPaymentNew(gymId, gym){
  ...
  try{
    ...
    // C7 fix: Use gym's currency for Stripe Elements (based on gym's physical country)
    const _priceForInit=sgPrice(cs.selectedPass||'day');
    const initPrice=_priceForInit.amount;
    const initCurrency=cs.gym?.currency||cs.gym?.pricing?.currency||_priceForInit.currency||'gbp';

    // C7 fix: Use gym's country for Stripe default country, not visitor's timezone
    const userCountry=cs.gym?.country||'GB';
  ...
  <div style="min-height:100%;background:linear-gradient(180deg,#0a0f14 0%,#0d1117 100%);padding:20px 16px;padding-top:calc(env(safe-area-inset-top,12px) + 12px)">
    ...
    <div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06);border-radius:20px;padding:20px;margin-bottom:16px">
      <div style="display:flex;align-items:center;gap:14px;margin-bottom:16px">
        <div style="width:52px;height:52px;background:linear-gradient(135deg,#FF6D00,#E66200);border-radius:16px;display:flex;align-items:center;justify-content:center;font-size:24px">🏋️</div>
        <div style="flex:1">
...
function BookingSuccessPage(){
  ...
  // S5-H03 FIX: Day pass = 24hr, calendar event spans to next day same time
  const calEndDateObj=new Date(calDate.slice(0,4)+'-'+calDate.slice(4,6)+'-'+calDate.slice(6,8));
  ...
  <div class="pt-16 min-h-full px-4 pb-8">
    <div class="max-w-lg mx-auto">
        ...
        <div class="space-y-4">
          ...
          <div class="flex gap-3">
            ...
            <div>
              ...
              <p class="text-slate-400 text-xs">${state.lastAccess?'Use the QR/PIN/mobile key above to unlock the door':'Show your QR code at reception to check in'}</p>
            </div>
          </div>
          <div class="flex gap-3">
      ...
      <div class="mt-6 text-center space-y-1">
        <p class="text-slate-500 text-xs">${(b.paymentMethod||'').includes('cash')?'💵 Cash payment · ':'🔒 Payment secured by Stripe · '}Free cancellation up to 2 hours before</p>
...
//  Shows real-time earnings, clicks, conversions for ScanSquad creators
// ═══════════════════════════════════════════════════════════════════
...
<div class="mb-8">
  ...
  <div class="grid sm:grid-cols-3 gap-4">
    <div class="bg-slate-800/50 rounded-xl p-5 text-center">
      ...
      <p class="text-slate-400 text-sm">Same price any time of day. Book whenever suits you.</p>
    </div>
    <div class="bg-slate-800/50 rounded-xl p-5 text-center">
      <div class="text-3xl mb-3">📊</div>
...
`);
    ...
    <div class="max-w-2xl mx-auto px-4 py-16">
      <div class="text-center mb-10">
        <h1 class="font-brand text-4xl font-bold text-white mb-3">Join ScanSquad</h1>
  ...
  else if(path==='/weight-calc')page=InfoPage('Weight Calculator',`<div style="padding:16px 0"><p style="text-align:center;font-size:48px">🏋️</p><h2 style="color:#fff;font-size:22px;font-weight:800;text-align:center;margin:12px 0">1RM Calculator</h2><div style="padding:0 8px"><label style="color:rgba(255,255,255,.5);font-size:13px;font-weight:600;display:block;margin-bottom:4px">Exercise</label><input id="sg-wc-exercise" type="text" placeholder="e.g. Bench Press" style="width:100%;padding:12px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);border-radius:10px;color:#fff;font-size:15px;margin-bottom:12px;box-sizing:border-box;outline:none"><div style="display:flex;gap:12px;margin-bottom:12px"><div style="flex:1"><label style="color:rgba(255,255,255,.5);font-size:13px;font-weight:600;display:block;margin-bottom:4px">Weight (kg)</label><input id="sg-wc-weight" type="number" placeholder="80" style="width:100%;padding:12px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);border-radius:10px;color:#fff;font-size:15px;box-sizing:border-box;outline:none"></div><div style="flex:1"><label style="color:rgba(255,255,255,.5);font-size:13px;font-weight:600;display:block;margin-bottom:4px">Reps</label><input id="sg-wc-reps" type="number" placeholder="8" style="width:100%;padding:12px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);border-radius:10px;color:#fff;font-size:15px;box-sizing:border-box;outline:none"></div></div><button onclick="(async function(){var w=parseFloat(document.getElementById('sg-wc-weight').value);var r=parseInt(document.getElementById('sg-wc-reps').value);var ex=document.getElementById('sg-wc-exercise').value;if(!w||!r){alert('Enter weight and reps');return;}var res=await fetch('/api/ai/weight-calculator',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({exercise:ex,weight:w,reps:r,unit:'kg'})});var d=await res.json();var h='<div style=\\'margin-top:16px\\'><div style=\\'background:rgba(255,109,0,.08);border:1px solid rgba(255,109,0,.2);border-radius:16px;padding:16px;text-align:center;margin-bottom:12px\\'><p style=\\'color:rgba(255,255,255,.4);font-size:12px\\'>Estimated 1RM</p><p style=\\'color:#FF6D00;font-size:36px;font-weight:900\\'>'+d.estimated1RM.average+' kg</p></div>';Object.keys(d.trainingZones).forEach(function(z){var zone=d.trainingZones[z];h+='<div style=\\'display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid rgba(255,255,255,.05)\\' ><span style=\\'color:#fff;font-size:14px;font-weight:600;text-transform:capitalize\\'>'+z+'</span><span style=\\'color:rgba(255,255,255,.5);font-size:13px\\'>'+zone.weight+' kg × '+zone.reps+' ('+zone.rest+')</span></div>';});h+='<div style=\\'margin-top:16px;background:rgba(59,130,246,.08);border:1px solid rgba(59,130,246,.2);border-radius:12px;padding:12px\\'><p style=\\'color:#3b82f6;font-size:12px;font-weight:700\\'>NEXT SESSION</p><p style=\\'color:rgba(255,255,255,.7);font-size:13px;margin-top:4px\\'>Option A: '+d.progressiveOverload.option1.weight+' kg × '+d.progressiveOverload.option1.reps+'</p><p style=\\'color:rgba(255,255,255,.5);font-size:12px;margin-top:2px\\'>'+d.progressiveOverload.option1.note+'</p></div></div>';document.getElementById('sg-wc-result').innerHTML=h;})()" style="width:100%;padding:14px;background:#FF6D00;color:#fff;border:none;border-radius:12px;font-size:16px;font-weight:700;cursor:pointer">Calculate 💪</button><div id="sg-wc-result"></div></div></div>`);
  // #84: Facility science
  else if(path==='/facility-science')page=InfoPage('Facility Science',`<div style="padding:16px 0;text-align:center"><p style="font-size:48px">🔬</p><h2 style="color:#fff;font-size:22px;font-weight:800;margin:12px 0">Scientific Benefits</h2><p style="color:rgba(255,255,255,.4);font-size:14px;margin-bottom:20px">Evidence-based benefits of gym facilities</p><div style="text-align:left">${['swimming-pool','sauna','free-weights','cardio-area','yoga-studio','boxing-ring'].map(function(f){return '<div onclick="navigate(\\'/facility-science/'+f+'\\' )" style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:14px 16px;margin-bottom:8px;cursor:pointer;display:flex;align-items:center;justify-content:space-between"><span style="color:#fff;font-size:14px;font-weight:600;text-transform:capitalize">'+f.replace(/-/g,' ')+'</span><span style="color:rgba(255,255,255,.2)">›</span></div>';}).join('')}</div></div>`);
  else if(path?.startsWith('/facility-science/'))page=(function(){var fType=path.split('/')[2];return InfoPage(fType.replace(/-/g,' '),`<div id="sg-fs-data" style="padding:16px 0"><p style="color:rgba(255,255,255,.3)">Loading...</p></div><script>setTimeout(async function(){var r=await fetch('/api/ai/facility-science/'+encodeURIComponent('${fType}'));var d=await r.json();var h='<h2 style="color:#fff;font-size:20px;font-weight:800;margin-bottom:4px">'+d.name+'</h2><p style="color:rgba(255,255,255,.3);font-size:13px;margin-bottom:16px">🔥 '+d.calories+' calories/session</p>';(d.benefits||[]).forEach(function(b){h+='<div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06);border-radius:16px;padding:16px;margin-bottom:10px"><p style="color:#FF6D00;font-size:14px;font-weight:700;margin-bottom:4px">'+b.title+'</p><p style="color:rgba(255,255,255,.7);font-size:13px;line-height:1.5">'+b.detail+'</p><p style="color:rgba(255,255,255,.25);font-size:11px;margin-top:6px;font-style:italic">📚 '+b.source+'</p></div>';});document.getElementById('sg-fs-data').innerHTML=h;},300)</script>`);})();
  // #85: Equipment tutorials
  else if(path==='/tutorials')page=InfoPage('Equipment Tutorials',`<div id="sg-tut-list" style="padding:16px 0"><p style="color:rgba(255,255,255,.3)">Loading...</p></div><script>setTimeout(async function(){var r=await fetch('/api/ai/equipment-tutorials');var d=await r.json();var h='<p style="text-align:center;font-size:48px;margin-bottom:12px">🎓</p><h2 style="color:#fff;font-size:22px;font-weight:800;text-align:center;margin-bottom:20px">Equipment Guides</h2>';(d.tutorials||[]).forEach(function(t){h+='<div onclick="navigate(\\'/tutorials/'+t.id+'\\' )" style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:14px 16px;margin-bottom:8px;cursor:pointer;display:flex;align-items:center;gap:14px"><div style="width:40px;height:40px;background:rgba(255,109,0,.1);border-radius:12px;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:18px">🏋️</div><div style="flex:1"><p style="color:#fff;font-size:14px;font-weight:600;margin:0">'+t.name+'</p><p style="color:rgba(255,255,255,.35);font-size:11px;margin:2px 0 0">'+t.muscles.join(', ')+' · '+t.difficulty+'</p></div><span style="color:rgba(255,255,255,.2)">›</span></div>';});document.getElementById('sg-tut-list').innerHTML=h;},300)</script>`);
...
function _injectLocationBanner(permState){
  ...
  if(isDenied){
    ...
    banner.innerHTML=''
      ...
      +'<p style="color:rgba(253,230,138,.7);font-size:11px;margin:2px 0 0;line-height:1.3;">Tap to find gyms near you</p>'
      +'</div>'
      +'<span onclick="event.stopPropagation();_dismissLocationBanner()" style="color:rgba(253,230,138,.5);font-size:18px;padding:4px 2px;cursor:pointer;flex-shrink:0;">✕</span>';
  ...
  fetch('/api/streaks',{credentials:'include'}).then(r=>r.json()).then(data=>{
    ...
    <div style="background:linear-gradient(135deg,rgba(255,109,0,.1),rgba(255,109,0,.03));border:1px solid rgba(255,109,0,.15);border-radius:16px;padding:16px;margin:12px 0">
      ...
      <div style="background:rgba(255,255,255,.06);border-radius:8px;height:8px;overflow:hidden;margin-bottom:6px">
        <div style="background:linear-gradient(90deg,#FF6D00,#E66200);height:100%;width:${Math.min(progress,100)}%;border-radius:8px;transition:width .5s ease"></div>
      </div>
...
window.sgVariableReferral=function(){
  // Variable mystery reward — amounts based around day-pass price
  const dp=sgPrice('day').display;
  const amounts=['£1','£2','£3',dp,dp,dp,'£10','£25'];
...
window.sgSunkCostBanner=function(container){
  ...
  fetch('/api/streaks',{credentials:'include'}).then(r=>r.json()).then(d=>{
    ...
    const spent=workouts*sgPrice('day').amount; // dynamic day-pass price
...
window.sgAutoplayNextGym=function(currentGymId){
  ...
  progress.style.cssText='position:absolute;bottom:0;left:0;height:3px;background:linear-gradient(90deg,#FF6D00,#E66200);border-radius:0 0 16px 16px;transition:width 1s linear;width:100%';
  ...
  return`
    <div style="margin-bottom:16px">
      ...
      <div style="height:4px;background:rgba(255,255,255,.08);border-radius:4px;overflow:hidden">
        <div style="height:100%;width:${pct}%;background:linear-gradient(90deg,#FF6D00,#E66200);border-radius:4px;transition:width .4s ease-out"></div>
      </div>
    </div>
  `;
};
...
window.sgFomoSpots=function(gymName){
  ...
  return`<div style="display:flex;align-items:center;gap:4px;font-size:11px;color:#ef4444;font-weight:600;margin-top:6px">
    <span style="animation:pulse 2s infinite">⚠️</span> Only ${spots} spots left today
  </div>`;
};
    ...
    container.innerHTML=`
      <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:16px;padding:16px;margin-bottom:12px">
        ...
        <div style="height:6px;background:rgba(255,255,255,.08);border-radius:6px;overflow:hidden;margin-bottom:12px">
          <div style="height:100%;width:${pct}%;background:linear-gradient(90deg,#FF6D00,#E66200);border-radius:6px;transition:width .4s"></div>
        </div>
...
// ─── 24. BEGINNER'S LUCK — PERMANENTLY REMOVED (C7 fix) ───
