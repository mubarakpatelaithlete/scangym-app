...
window._sgB3VerifyOwnership=async function(targetGymId){
  ...
  window._sgOpenSheet('sg-own-sheet',head
    ...
    +'<div id="sg-own-preview" style="font-size:12px;color:rgba(255,255,255,.4);text-align:left;line-height:1.5;margin:0 0 16px">Checking your gym\u2019s registered number\u2026</div>'
...
window._sgB3PhonePreview=async function(gymId){
  ...
  try{
    ...
    if(d.hasNumber){
      ...
      box.innerHTML='<span style="color:#fbbf24">\u26A0\uFE0F '+(d.message||'We do not have a phone number for your gym yet.')+'</span><div style="margin-top:3px">'+(d.action||'Upload proof of ownership below instead.')+'</div>';
      var send=document.getElementById('sg-own-send');
      if(send){send.style.opacity='.45';send.title='No registered number for your gym yet';}
    }
